...
# Save overall data for external validity check
data_all <- data
...
# Plot price index
gl1 <- ggplot(data = data, aes(x = as.Date(date), y = p))+
  geom_line() +
...
gd1 <- ggplot(data = data, aes(x = as.Date(date), y = dp))+
  geom_line() +
...
gdl1 <- ggplot(data = data, aes(x = as.Date(date), y = dlnp))+
  geom_line() +
...
# 
# Plot employment
gl2 <- ggplot(data = data, aes(x = as.Date(date), y = emp))+
  geom_line() +
...
gdl2 <- ggplot(data = data, aes(x = as.Date(date), y = dlnemp))+
  geom_line() +
...
gl3 <- ggplot(data = data, aes(x = as.Date(date), y = u))+
  geom_line() +
...
gdl3 <- ggplot(data = data, aes(x = as.Date(date), y = du))+
  geom_line() +
...
##
# ARRANGE GRAPHS INTO ONE 
# levels
g_levels <- ggarrange(gl1, gl2, gl3,
                              hjust = -0.6,
                              ncol = 1, nrow = 3)
g_levels

#log differences
g_logdiff <- ggarrange(gdl1, gdl2, gdl3,
                      hjust = -0.6,
                      ncol = 1, nrow = 3)
g_logdiff
...
# Define the samples to test for cv
...
# Note: TSLM is not an easy and intuitive object to work with...
# more can be found on: https://otexts.com/fpp3/index.html
#   it is a great material but sometimes a bit too technical

# M2 p ~ auto ARIMA without seasonal arima components
m2_pre <- data_work %>%
  model(auto_arima = ARIMA(p ~  PDQ(0,0,0)))

m2_pre
# get the AR(p)I(d)MA(q) parameters
...
# if you take the differenced outcome it is more cumbersome to
#   'integrate' back -> cumsum
model_5 <- data_tr %>%
  model(m5 = m5)
...
#   same with log transformation -> need to adjust
model_6 <- data_tr %>%
  model(m6 = m6)
...
# Last year of data
data_holdout_2<- data_all %>%
  slice((n()-11):n())

# Rest of data for work set
data_work_2 <- data_all %>%
  slice(1:(n()-12))
...
# graph actual vs prediction from best arima
data_plot <-  left_join(data_all,forecast_holdout_best) %>%
  filter(year(date)>=2015)
